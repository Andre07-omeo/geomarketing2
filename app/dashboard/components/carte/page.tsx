...
function ReservationDetailModal({ reservationData, panneau, onClose, user }: any) {
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (uploadedUrl) {
      ...
      setModificationMessage("✓ Photo téléchargée, cliquez sur Enregistrer");
    } else {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updatedReservation = {
        ...
        statut: newStatut || reservationsToUpdate[reservationIndex].statut,
          ...
          <div className="p-5 space-y-4 max-h-[60vh] overflow-y-auto custom-scrollbar">
            ...
            {modificationMessage && (
              <div className={`p-3 rounded-xl text-center text-xs font-medium ${
                modificationMessage.includes('succès') 
            ...
            <div className="bg-white/5 rounded-xl p-3">
              ...
              <div className="mt-2">
                {(localPreview || newPhotoUrl || res.photoCampagneUrl) && !res.photoCampagneUrl?.includes('dispromalt') ? (
                  <div className="space-y-2">
                    ...
                    {isEditing && (
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-2 py-2 rounded-lg bg-amber-500/20 border border-amber-500/30 text-amber-400 text-xs font-bold hover:bg-amber-500/30 transition-all"
                      >
                        <Camera size={14} />
                        Changer la photo
                      </button>
                  ...
                  <button
                    onClick={() => fileInputRef.current?.click()}
                    className="w-full flex flex-col items-center justify-center gap-2 py-6 rounded-lg border-2 border-dashed border-white/20 hover:border-amber-500/50 transition-all"
                  >
                    <Camera size={24} className="text-white/40" />
                    <span className="text-xs text-white/40">Ajouter une photo</span>
                  </button>
                )}
                <input
                  ref={fileInputRef}
